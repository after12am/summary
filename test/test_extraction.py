...
from summary.extractor import extract
...
def calculate_similarity(a, b):
    _a = unicode(a)
    _b = unicode(b)
    _a = re.compile(r'\s').sub(u'', _a)
    _b = re.compile(r'\s').sub(u'', _b)
    return difflib.SequenceMatcher(None, _a, _b).ratio()


# english pages
class TestENContent(unittest.TestCase):

    def setUp(self):
        self.threshold = .9
    
    def test_title_of_music_visualizer_progress(self):
        expected = u"Music visualizer progress"
        title = extract(uri = uri_en['music_visualizer_progress']).title
        assert title is not False
        assert calculate_similarity(title, expected) > self.threshold
    
    def test_text_of_music_visualizer_progress(self):
        expected = u"""Actually something pretty cool happened today.. I wanted to control the sketch params with a GUI, so I quickly coded one with controlP5 that was sending OSC messages to the rendering sketch..
        ...
        """
        text = extract(uri = uri_en['music_visualizer_progress']).text
        assert text is not False
        assert calculate_similarity(text, expected) > self.threshold
    
    def test_title_of_abstrakt_abstrakt_jorinde_voigt(self):
        expected = u"» abstrakt Abstrakt: Jorinde Voigt"
        title = extract(uri = uri_en['abstrakt_abstrakt_jorinde_voigt']).title
        assert title is not False
        assert calculate_similarity(title, expected) > self.threshold
    
    def test_text_of_abstrakt_abstrakt_jorinde_voigt(self):
        expected = u"""Jorinde Voigt: Territorium (4), VI/aus: Position 1-x; Nord-Süd-Achse; Zentren A-Z; Position-Zentrum/Identisch; Territorium 1-x; Zentrum 1-x; N,S,W,O; Drehrichtung der Himmelsrichtung im Verlauf; Konstruktion; Dekonstruktion; Countup-Countdown-Loop: 1-x Tage; Kontinentalgrenze
        Rome, 2010
        ...
        """
        text = extract(uri = uri_en['abstrakt_abstrakt_jorinde_voigt']).text
        assert text is not False
        assert calculate_similarity(text, expected) > self.threshold


# japanese pages
class TestJAContent(unittest.TestCase):

    def setUp(self):
        self.threshold = .9

    def test_title_of_post_10869(self):
        expected = u"今まで見た中で1番美しいタイムラプス動画（動画）"
        title = extract(uri = uri_ja['post_10869']).title
        assert title is not False
        assert calculate_similarity(title, expected) > self.threshold
    
    def test_text_of_post_10869(self):
        expected = u"""写真家で米Gizmodoの読者でもあるジョン・イークルンド（John Eklund）さんが、この桁外れに美しいタイムラプス動画を送ってきてくれました。撮影地は太平洋岸北西部です。
        ...
        Jesus Diaz（原文／mio）
        """
        text = extract(uri = uri_ja['post_10869']).text
        assert text is not False
        assert calculate_similarity(text, expected) > self.threshold
    
    def test_title_of_nakatani_web_1(self):
        expected = u"Webページの本文抽出"
        title = extract(uri = uri_ja['nakatani_web_1']).title
        assert title is not False
        assert calculate_similarity(title, expected) > self.threshold
    
    def test_text_of_nakatani_web_1(self):
        expected = u"""Webページの自動カテゴライズ の続き。
        ...
        """
        text = extract(uri = uri_ja['nakatani_web_1']).text
        assert text is not False
        assert calculate_similarity(text, expected) > self.threshold
